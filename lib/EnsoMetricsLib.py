...
def EnsoRmse(modelVar1, modelNameVar1, obsVar1, obsNameVar1, centered_rmse=0, **kwargs):
    ...
    """
    # get
    mod_tab, mod_axes, mod_weights, mod_Nyears = modelVar1['array'], modelVar1['axes'], modelVar1['area_weights'], \
                                                 modelVar1['Nyears']
    obs_tab, obs_axes, obs_weights, obs_Nyears = obsVar1['array'], obsVar1['axes'], obsVar1['area_weights'], \
                                                 obsVar1['Nyears']
    ...
    if isinstance(kwargs['min_number_of_years'], int):
        ...
        if mod_Nyears < mini:
            EnsoErrorsWarnings.TooShortTimePeriod("EnsoRmse", mod_Nyears, mini, INSPECTstack(), info=modelNameVar1)
        if obs_Nyears < mini:
            EnsoErrorsWarnings.TooShortTimePeriod("EnsoRmse", obs_Nyears, mini, INSPECTstack(), info=obsNameVar1)
