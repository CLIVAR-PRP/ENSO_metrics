...
def find_xml_cmip(model, project, experiment, ensemble, frequency, realm, variable):
    try: pathnc, filenc = find_path_and_files(ens=ensemble, exp=experiment, fre=frequency, mod=model, pro=project, rea=realm, var=variable)
        ...
        try: pathnc, filenc = find_path_and_files(ens=ensemble, exp=experiment, fre=frequency, mod=model, pro=project, rea=new_realm, var=variable)
            ...
            print '%%%%%     -----     %%%%%' + bcolors.ENDC
            sys.exit('')
        file_area, file_land = find_fx(model, project, experiment, ensemble, new_realm)
    else:
        file_area, file_land = find_fx(model, project, experiment, ensemble, realm)
...
def find_xml_fx(name, project='', experiment='', realm=''):
    list_obs = ReferenceObservations().keys()
    if name in list_obs:
        file_area = OSpath__join(xmldir, 'obs_' + str(name) + '_glob_fx_O_areacell.xml')
        file_land = OSpath__join(xmldir, 'obs_' + str(name) + '_glob_fx_O_landmask.xml')
    else:
...
if mc_name == 'MC1':
    ...
    list_obs = ['Tropflux','GPCPv2.3']#['HadISSTv1.1']#['HadISSTv1.1','Tropflux','GPCPv2.3']#['Tropflux','GPCPv2.3']
...
for mod in list_models:
    ...
    netcdf_name = today + '_YANN_PLANTON_' + mc_name + '_' + mod + '_' + experiment + '_' + ensemble
