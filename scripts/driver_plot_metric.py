...
# list metrics
list_metrics = ["telecon_pr_djf"]
# list observations
list_observations = ["20CRv3", "OISSTv2_GPCPv2.3"]
# ---------------------------------------------------#
...
for met in list_metrics:
    for obs in list_observations:
        ...
        json_file = list(GLOBiglob(OSpath__join(path, json_file)))[0]
        ...
        netcdf_file = list(GLOBiglob(OSpath__join(path, netcdf_file)))[0]
        ...
        # figure name
        name_png = OSpath__join(path, figure_pattern.replace("METRIC", met) + "_" + str(obs2))
        # plot
        plotter_1mod_1obs(
